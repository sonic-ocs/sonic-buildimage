...
start() {
    ...
    TOPOLOGY_SCRIPT="/usr/share/sonic/device/$PLATFORM/plugins/config-topology-init.sh"
    chmod +x $TOPOLOGY_SCRIPT
...
stop() {
    PLATFORM=${PLATFORM:-`sonic-cfggen -H -v DEVICE_METADATA.localhost.platform`}
    #Path to platform topology script
    TOPOLOGY_SCRIPT="/usr/share/sonic/device/$PLATFORM/plugins/config-topology-destroy.sh"
    #if topology script file not present, do nothing and return 0
    [ ! -f $TOPOLOGY_SCRIPT ] && exit 0
    $TOPOLOGY_SCRIPT
}
